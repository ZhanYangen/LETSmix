...
class LETSmix(nn.Module):
    def __init__(self, dims, lr, d=0.5, act=nn.ELU()):
        ...
        ----------
        dims : list
            The dimensions of the input, two hidden layers in the feature extractor, \
                one hidden layer in the domain classifier, and the output dimension of the source classifier.
        lr : dict
            The learning rate for the source classifier and domain classifier.
        d : float, optional
            The dropout rate for the domain classifier. The default is 0.5.
        act : nn.Module, optional
            The activation function for the feature extractor. The default is nn.ELU().
