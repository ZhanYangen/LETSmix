...
def load_data(args):
    ...
    loader['t_val'] = DataLoader(dataset, args.bs['target'], False, num_workers=8)
    ...
    idx = np.arange(len(mat_sp_s))
    np.random.shuffle(idx)
    mat_sp_s = torch.from_numpy(mat_sp_s[idx[:int(args.r*len(idx))]])
    loader['train'] = DataLoader(dataset, args.bs['source'], True, num_workers=8, drop_last=True,
                                collate_fn=lambda batch: collate_fn(batch, mat_sp_s, args))
    params = {'dim_in':mat_sc_s.shape[1], 'dim_out':len(sc_sub_dict2)}
    return loader, adata_st, params, sc_sub_dict2
