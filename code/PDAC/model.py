...
class SpaDA(nn.Module):
    ...
    def forward(self, x, y=None, yd=None, mode='train', alpha=1):
        z = self.fe(x)
        if mode == 'pretrain':  # ignore the domain classifier
            ...
            pred_d = self.dc(z)
            pred = self.sc(z)
            loss_d = F.cross_entropy(pred_d, yd)
            loss_s = F.kl_div(torch.log(pred[:len(y)]), y, reduction='batchmean')
            loss = loss_s + alpha*loss_d
            self.opt.zero_grad()
            loss.backward()
            self.opt.step()
            return loss_s.item()
